...
const Dashboard: React.FC = () => {
  ...
  const generatePieChartData = (categoryName: string) => {
    const relevantCategories =
      categoryName === "all"
        ? currentCategories
        : currentCategories.filter((cat) => cat.name === categoryName);
        ...
        const divisionArahan = allArahan.filter(
          (arahan) =>
            ...
            relevantCategories.some(
              (cat) =>
                arahan.title.toLowerCase().includes(cat.name.toLowerCase()) ||
                arahan.description
                  .toLowerCase()
  ...
  const generateCategoryTablePieChartData = (categoryTable: CategoryTable) => {
    const divisionData = categoryTable.tableData.map((data) => {
      ...
      return {
        name: data.division,
        value: data.progress, // Progress percentage for pie chart
        total: data.jumlah,
        selesai: data.selesai,
        selesaiBerkelanjutan: data.selesaiBerkelanjutan,
        proses: data.proses,
        belumDitindaklanjuti: data.belumDitindaklanjuti,
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value, name) => [`${value} arahan`, name]}
                  />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
