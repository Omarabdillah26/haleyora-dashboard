...
app.get('/api/category-table-data/:categoryId', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to get category table data', error: error.message });
  }
});

app.post('/api/category-table-data', async (req, res) => {
  try {
    const { categoryId, division, jumlah, proses, selesai, belumDitindaklanjuti, selesaiBerkelanjutan, progress } = req.body;
    ...
    const [result] = await pool.execute(
      ...
      [id, categoryId, division, jumlah, proses, selesai, belumDitindaklanjuti, selesaiBerkelanjutan, progress]
    );
    ...
    res.status(500).json({ success: false, message: 'Failed to create category table data', error: error.message });
  }
});
...
app.get('/api/arahans', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to get arahans', error: error.message });
  }
});
...
app.post('/api/arahans', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to create arahan', error: error.message });
  }
});
...
app.put('/api/arahans/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to update arahan', error: error.message });
  }
});
...
app.delete('/api/arahans/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await pool.execute('DELETE FROM arahans WHERE id = ?', [id]);
    ...
    res.status(500).json({ success: false, message: 'Failed to delete arahan', error: error.message });
  }
});
